    ...
    PaymentSplitter 
{
    ...
    address[] recipients_ = [
        0x3dF6149A499A6248CA756e611b849D912058C435,
        0x6b79F11CF5E2f6db10DC694e7a8B2ecCeB12bb52
    ];
    uint256[] shares_ = [
        925,
        75
    ...
    function whitelistMint(uint64 count, uint64 capacity, bytes32[] memory proof) external payable nonReentrant notBots whenNotPaused {
        require(count > 0, "NoZero");
        require(whitelistLive, "NotLive");
        require(whitelist.isPermitted(msg.sender, capacity, proof), "notWhitelisted");
        require(msg.value >= config.wlPrice * count, "invalidPrice");
        uint64 newCount = _getAux(msg.sender) + count;
        require(newCount <= capacity, "ExceedsMax");        
    ...
    modifier notBots {        
        require(_msgSender() == tx.origin, "noBot");
        _;
    }
    ...
    function _callMint(uint256 count, address to) internal {        
        ...
        require(total + count <= supply, "SoldOut");
        _safeMint(to, count);
    }
    ...
    {
        require(_exists(tokenId), "NoExtTkn");
        return
    ...
    function setUriProperty(string memory _extension, string memory _uri) external adminOrOwner {
        ...
        unchecked {
            for (uint256 i; i < _currentIndex; i++) {
                ...
                if (_ownerships[i].addr != address(0)) {
                    currOwnershipAddr = _ownerships[i].addr;
                }
    ...
    modifier adminOrOwner() {
        require(msg.sender == owner() || admins[msg.sender], "Unauth");
        _;
    }
}